...
class ActorCriticDWAQPerception(nn.Module):
    is_recurrent = False
    ...
    def __init__(
        ...
        point_feat_dim=4,
        pointnet_shared_mlp_dims=[64, 128, 256],
        pointnet_latent_dim=32,
        use_confidence_filter=True,
        mask_origin=True,
        origin_eps=1e-3,
        heightmap_recon_dim=187,
        heightmap_decoder_hidden_dims=[128, 128],
        mixer_token_dim=128,
        ...
        **kwargs,
        ...
        for key, dim in self.obs_hist_dict.items():
            if key != "height_scan":
                self.current_indices += list(
                    range(sum + (self.history_length - 1) * dim, sum + self.history_length * dim)
                )
            sum += dim * self.history_length
        ...
        self.heightmap_decoder = HeightmapDecoder(
            latent_dim=pointnet_latent_dim,
            out_dim=heightmap_recon_dim,
            hidden_dims=heightmap_decoder_hidden_dims,
            activation=activation,
        )
        # fuse the proprioceptive CENet latent (cenet_out_dim) with the exteroceptive latent
        self.mixer = MLPMixerFusion(
            proprio_dim=cenet_out_dim,
            extero_dim=pointnet_latent_dim,
            token_dim=mixer_token_dim,
            hidden_dim=mixer_hidden_dim,
            num_blocks=mixer_num_blocks,
        )

        # actor: consumes the fused (proprio+extero) latent concatenated with the current proprio obs
        self.actor = MLP(num_actor_obs + self.mixer.out_dim, num_actions, actor_hidden_dims, activation)
        # actor observation normalization (applies to the current proprio obs only)
    ...
    def get_point_cloud_obs(self, obs):
        obs_list = []
        for obs_group in self.obs_groups[self.point_cloud_group]:
            obs_list.append(obs[obs_group])
        return torch.cat(obs_list, dim=-1)

    def perception_forward(self, obs):
        """Encode the accumulated point cloud and reconstruct the privileged heightmap.

        Returns ``(recon_heightmap, mean_pe, logvar_pe)`` for the (deferred) exteroceptive VAE loss in
    ...
    def get_height_scan_target(self, obs):
        """Privileged heightmap reconstruction target.

        Prefers a dedicated ``height_scan`` obs set; otherwise falls back to the last
        ``heightmap_decoder.out_dim`` dims of the (unnormalized) critic obs.
        """
        if "height_scan" in self.obs_groups:
            obs_list = [obs[obs_group] for obs_group in self.obs_groups["height_scan"]]
            return torch.cat(obs_list, dim=-1)
        return self.get_critic_obs(obs, normalize=False)[:, -self.heightmap_decoder.out_dim :]
