...
from rsl_rl.modules import ActorCriticDWAQPerception
...
class PPODreamWAQPerception:
    """Proximal Policy Optimization algorithm (https://arxiv.org/abs/1707.06347)."""

    policy: ActorCriticDWAQPerception
    ...
    def update(self, beta=None):  # noqa: C901
        ...
        mean_perception_loss = 0
        mean_perception_reconstruction_loss = 0
        mean_perception_kld_loss = 0
        # -- RND loss
        if self.rnd:
            mean_rnd_loss = 0
        else:
        ...
        for (
            ...
            # Beta VAE loss
            code, code_vel, decode, mean_vel, logvar_vel, mean_latent, logvar_latent = self.policy.cenet_forward(
                self.policy.get_history_obs(obs_batch)
            )
            ...
            # Exteroceptive (DreamWaQ++) VAE loss: reconstruct the privileged heightmap from the
            # point-cloud latent. This supervises the PointNet latent to encode foot-relevant terrain.
            if self._has_perception:
                recon_heightmap, mean_pe, logvar_pe = self.policy.perception_forward(obs_batch)
                heightmap_target = self.policy.get_height_scan_target(obs_batch)
                heightmap_target.requires_grad = False
                perception_reconstruction_loss = (
                    nn.MSELoss(reduction="sum")(recon_heightmap, heightmap_target) / obs_batch.shape[0]
                )
                perception_kld_loss = (-0.5 * torch.sum(1 + logvar_pe - mean_pe.pow(2) - logvar_pe.exp(), dim=1)).mean(
                    dim=0
                )
                perception_beta = getattr(self.policy, "perception_beta", beta)
                perception_loss = perception_reconstruction_loss + perception_beta * perception_kld_loss
            else:
                perception_loss = torch.zeros((), device=self.device)
                perception_reconstruction_loss = perception_loss
                perception_kld_loss = perception_loss
            ...
            mean_perception_kld_loss += perception_kld_loss.item()
        ...
        mean_perception_kld_loss /= num_updates
        ...
        loss_dict = {
            ...
            "perception_kld": mean_perception_kld_loss,
        }
        if self.rnd:
